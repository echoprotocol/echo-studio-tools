...
var CoderAddress = /** @class */ (function (_super) {
    ...
    CoderAddress.prototype.encode = function (value) {
        var result = new Uint8Array(32);
        try {
            result.set(bytes_1.arrayify(address_1.getAddress(value)));
        }
        catch (error) {
    ...
    CoderAddress.prototype.decode = function (data, offset) {
        var output = bytes_1.hexlify(data.slice(offset, offset + 32));
        if (!/^(0x)?0{25}(0|1)[a-f\d]{38}$/.test(output)) {
        ...
        var isContract = output[25] === '1';
        var index = Number.parseInt(output.substr(26), 16);
        return {
            consumed: 32,
            value: this.coerceFunc('address', ['1', isContract ? '14' : '2', index.toString(10)].join('.'))
        };
    };
    return CoderAddress;
}(Coder));
